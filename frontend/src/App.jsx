...
export default function App() {
  ...
  useEffect(() => {
    if (token) {
      // 1. Do an initial fetch of the data
      fetchItems();

      // 2. Open a permanent tunnel to the Daphne server
      const ws = new WebSocket('ws://127.0.0.1:8000/ws/items/');

      // 3. Listen for broadcasts from the Radio Tower
      ws.onmessage = (event) => {
        const message = JSON.parse(event.data);
        if (message.type === 'update_matrix') {
          fetchItems(); // Instantly refresh the table!
        }
      };

      // 4. Close the tunnel securely if the user logs out
      return () => {
        ws.close();
      };
    }
  ...
  const handleLogin = async (e) => {
    ...
    toast.promise(loginPromise, {
      ...
      success: (data) => {
        // JWT uses 'access' instead of 'token'
  ...
  const handleSubmit = async (e) => {
    ...
    const submitPromise = fetch(API_URL, {
      method: 'POST',
      // JWT requires the word 'Bearer' instead of 'Token'
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-8 font-sans">
      ...
      <header className="mb-8 flex justify-between items-center border-b pb-4">
        ...
        {token && (
          <button onClick={handleLogout} className="bg-slate-600 text-white px-4 py-2 rounded shadow hover:bg-slate-700 transition">
            Exit Admin Session
          </button>
        )}
      </header>
      ...
      <div className="grid grid-cols-1 gap-8">
        {!token ? (
          <div className="bg-white p-6 rounded-xl shadow-md max-w-md mx-auto w-full mt-10">
            <h2 className="text-xl font-bold mb-4 text-slate-700">Admin Authentication Gate</h2>
            <form onSubmit={handleLogin} className="space-y-4">
              <div>
                <label className="block text-sm font-semibold mb-1 text-slate-600">Username</label>
                <input type="text" className="w-full border rounded p-2" value={username} onChange={e => setUsername(e.target.value)} required />
              </div>
              <div>
          ...
          <>
            <div className="bg-white p-6 rounded-xl shadow-md border-t-4 border-emerald-500">
              ...
              <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                ...
                <button type="submit" className="bg-emerald-600 text-white p-2 rounded font-bold hover:bg-emerald-700 transition h-10">
                  Add Record
                </button>
              </form>
            </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-50 border-b border-black">
                      ...
                      <th className="p-3 font-bold text-center text-black">Shapecolor</th>
                      ...
                      items.map(item => (
                        <tr key={item.id} className="border-b border-black hover:bg-slate-50 transition">
                          ...
                          <td className="p-2 text-center border-l border-slate-200 align-middle">
                            <button onClick={() => handleDelete(item.id)} className="text-rose-500 hover:bg-rose-50 px-2 py-1 rounded text-xs">✕</button>
                          </td>
                        </tr>
                      ))
                    )}
